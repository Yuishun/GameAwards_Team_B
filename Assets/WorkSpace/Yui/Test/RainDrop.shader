...
Shader "Unlit/RainDrops"
{
		...
		SubShader
	{
		...
		Pass
		{
		   ...
		   float4 tex(float2 i, float4 texCoord) {
			   return tex2Dlod(_MainTex, texCoord);
		   }

		   float3 rain(float2 i) {
			   float2 uv = ((i * _ScreenParams.xy) - .5*_ScreenParams.xy) / _ScreenParams.y;
			   float2 UV = i;
			   ...
			   float zoom = .3;// lerp(.3, 1.2, story);		// slowly zoom out
			   uv *= zoom;
